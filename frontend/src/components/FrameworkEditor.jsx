  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Get existing _raw data
        const existingRaw = frameworkData._raw || {}
        const existingFw = existingRaw.framework || existingRaw
        
        // Get the regenerated framework data
        const regeneratedFw = result.framework || {}
        
        // Preserve artefact_variants if they exist in the response, otherwise keep existing ones
        const artefactVariants = regeneratedFw.artefact_variants || 
                                 regeneratedFw.framework?.artefact_variants ||
                                 existingFw.artefact_variants || 
                                 []

        // Build the new _raw structure, ensuring artefact_variants is preserved
        const newFrameworkData = {
          ...existingFw,
          ...regeneratedFw,
          artefact_variants: artefactVariants,
        }

        // Determine the correct _raw structure
        let newRaw
        if (Object.prototype.hasOwnProperty.call(existingRaw, 'framework')) {
          newRaw = { ...existingRaw, framework: newFrameworkData }
        } else {
          newRaw = newFrameworkData
        }

        const updatedData = {
          ...frameworkData,
          steps: result.framework?.steps || frameworkData.steps,
          risks: result.framework?.risks || frameworkData.risks,
          escalation: result.framework?.escalation || frameworkData.escalation,
          confidence: result.framework?.confidence || frameworkData.confidence,
          _raw: newRaw,
          metadata: {
            ...frameworkData.metadata,
            title: result.framework?.title || frameworkData.metadata.title,
            lastUpdated: new Date().toISOString(),
          },
        }

        console.log('📦 Updated artefact_variants:', artefactVariants.length, 'items')
        ...
        return (
          <div>
            ...
            <div className="space-y-6">
              ...
              {/* Fourth row - Confidence Score */}
              <div className="mt-6">
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  Confidence Score
                  <span className="ml-2 text-xs text-gray-500 font-normal">
                    Quality and completeness indicator
                  </span>
                </label>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className="text-4xl font-bold text-blue-600">
                        {frameworkData.confidence 
                          ? Math.round(frameworkData.confidence) 
                          : 'N/A'}%
                      </div>
                      <div className="text-sm text-gray-600 mt-1">
                        Current Framework Confidence
                      </div>
                    </div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-500 ${
                            (frameworkData.confidence || 0) >= 80 
                              ? 'bg-green-500' 
                              : (frameworkData.confidence || 0) >= 65 
                              ? 'bg-yellow-500' 
                              : 'bg-red-500'
                          }`}
                          style={{ width: `${frameworkData.confidence || 0}%` }}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-blue-200 pt-4 mt-4">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <div className="flex items-start space-x-3 p-3 bg-white rounded-md">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-600 mt-0.5">
                            Framework is comprehensive, well-structured, and ready for deployment. All key sections are complete with detailed guidance.
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Medium Confidence */}
                      <div className="flex items-start space-x-3 p-3 bg-white rounded-md">
                        <div className="flex-shrink-0 w-2 h-2 mt-1.5 bg-yellow-500 rounded-full"></div>
                        <div className="flex-1">
                          <div className="text-sm font-semibold text-gray-800">
                            65-79% (Medium Confidence)
                          </div>
                          <div className="text-xs text-gray-600 mt-0.5">
                            Framework has good structure but may benefit from refinement. Consider adding more details or reviewing some sections.
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Low Confidence */}
                      <div className="flex items-start space-x-3 p-3 bg-white rounded-md">
                        <div className="flex-shrink-0 w-2 h-2 mt-1.5 bg-red-500 rounded-full"></div>
                        <div className="flex-1">
                          <div className="text-sm font-semibold text-gray-800">
                            Below 65% (Low Confidence)
                          </div>
                          <div className="text-xs text-gray-600 mt-0.5">
                            Framework needs significant development. Use the Re-generate button to improve quality or manually add more content.
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-blue-200">
                    <div className="flex items-start space-x-2">
                      ...
                      <p className="text-xs text-gray-600">
                        <strong>Note:</strong> Confidence scores are currently generated based on framework completeness and structure. 
                        The score reflects the initial quality of the source material and AI-generated content.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {artefactVariants.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm mt-1">
                        Edit the content directly using the rich text editor below. Use the toolbar to add tables, format text, and more.
                      </p>
                    </div>
                  </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-gray-500 text-sm">
                    Click <span className="font-semibold text-purple-600">"Re-generate"</span> below to generate artefact outlines using AI.
                  </p>
                </div>
              )}
            </div>
